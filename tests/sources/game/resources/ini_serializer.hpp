#ifndef GAME_RESOURCES_INI_SERIALIZER_HPP
#define GAME_RESOURCES_INI_SERIALIZER_HPP
...
#include "game/resources/serializer.hpp"
...
namespace game { namespace resources {
	...
	{
	private:
		typedef djah::data_object_t<AttributeTypes> data_object;
	...
	private:
		...
		//------------------------------------------------------------------------------------------
		template<>
        struct attribute_deserializer<djah::nulltype>
		{
        ...
        struct attribute_deserializer< djah::typelist<H, T> >
		{
			//--------------------------------------------------------------------------------------
			...
			{
                return djah::string_utils::to_lower_case(strVal) == "true";
			}
			//--------------------------------------------------------------------------------------
			template<>
			...
			{
				if( djah::resources::json_serializer<H>::is_of_type(attributeValue) )
				{
					H val;
                    djah::resources::json_serializer<H>::deserialize(attributeValue, val);
		...
		//------------------------------------------------------------------------------------------
		template<>
        struct attribute_serializer<djah::nulltype>
		{
            static bool execute(djah::filesystem::stream_sptr strm, const data_object_sptr &dobj)
			{
				return true;
			}
		};
		//------------------------------------------------------------------------------------------
		template<typename H, typename T>
        struct attribute_serializer< djah::typelist<H, T> >
		{
			//--------------------------------------------------------------------------------------
            static bool execute(djah::filesystem::stream_sptr strm, const data_object_sptr &dobj)
			{
	...
	public:
		//------------------------------------------------------------------------------------------
		static bool serialize(djah::filesystem::stream_sptr strm, const data_object_sptr &dobj)
		{
		...
		//------------------------------------------------------------------------------------------
		static bool deserialize(djah::filesystem::stream &stream, data_object_sptr dobj)
		{
			djah::filesystem::memory_stream memStream(&stream);
			const std::string &stringStream = memStream.toString();

            djah::string_utils::string_list_t lines;
            djah::string_utils::split_string(stringStream, lines, "\n");
			...
			{
                const std::string &trimmedLine = djah::string_utils::trimmed(*it);
				...
				if( !trimmedLine.empty() && trimmedLine[0] != '[' && trimmedLine[0] != '#' )
				{
					...
					if( spacePos != std::string::npos && equalPos != std::string::npos )
					{
                        ...
                        const std::string &attributeName = djah::string_utils::trimmed(trimmedLine.substr(spacePos + 1, equalPos - spacePos - 1));
                        const std::string &attributeValue = djah::string_utils::trimmed(trimmedLine.substr(equalPos + 1));
		...
		//------------------------------------------------------------------------------------------
		static bool deserialize2(djah::filesystem::stream &stream, data_object_sptr dobj)
		{
			djah::filesystem::memory_stream memStrm(&stream);
...
#endif /* GAME_RESOURCES_INI_SERIALIZER_HPP */