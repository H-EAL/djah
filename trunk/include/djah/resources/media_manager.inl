namespace djah { namespace resources {
	...
	void media_manager<ExtraTypes_, DefaultTypes_>::registerLoader(loader<T> *loader, const std::string &ext)
	{
		...
		typename media_holder<T>::loader_t_ptr p_loader(loader);
	...
	{
		T *res = 0;
		typename media_holder<T>::loader_t_ptr ldr = findLoader<T>(url);
		if(ldr)
		{
	...
	media_manager<ExtraTypes_, DefaultTypes_>::findLoader(const std::string &url)
	{
		...
		typename media_holder<T>::loader_t_ptr ldr;
		typename media_holder<T>::loaders_map_t::const_iterator it = media_holder<T>::loaders_.find(extension);
