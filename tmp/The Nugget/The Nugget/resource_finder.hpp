...
{
	static djah::resources::default_media_manager s_dmm;
	...
	std::shared_ptr<T> res = djah::resources::resource_manager::get().find<T>(url);
	// If not found try to load it through the media manager
	if( !res && (res = std::shared_ptr<T>(s_dmm.loadFromUrl<T>(url))) )
	{
		djah::resources::resource_manager::get().add(url, res);
	}
	else if( !res )
	{
